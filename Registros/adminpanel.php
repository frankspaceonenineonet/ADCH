<?php
	...
	$query="SELECT idpersona, nombre, email, telefono, idempresa, idpuesto, fecha FROM personal";
...
<head>
  ...
  <script type="text/javascript" src="../JS/JS_Registros/jquery-1.6.2.min.js"></script>
  <script type="text/javascript" src="../JS/JS_Registros/jquery-ui-1.8.16.custom.min.js"></script>
		...
		<form name="test" action="guardaregistro.php" onsubmit="return comprueba_dominio(test.valida_mail.value)" method="post" autocomplete="off">
            ...
            <select name="puesto" id="cboPuestos">
                <option value="0">Seleccione un Puesto</option>
                 ...
                 <br>               
            <button type="submit" class="btn btn-login" onclick="return confirm('Si no estas seguro(a) de que los datos sean correctos, favor de revisarlos. \n\nGrupo Nu3')"><strong>REGISTRAR</strong></button> 
                            </form>
</table>
						</div>
					<div class="wraper">
						<div class="informacion">
							<div id="dialogSencillo" class="btn btn-primary glyphicon glyphicon-plus" title="Agregar Nuevo Registro">
				...
				<tbody>
					<?php while($row=$resultado->fetch_assoc()){ ?>
						<tr>
							<td>
							<center><a href="modificar.php?idpersona=<?php echo $row['idpersona'];?>"><button type="button" class="btn btn-warning glyphicon glyphicon-pencil" title="Modificar Registro"></button></a></center>
							</td>
							<td>
